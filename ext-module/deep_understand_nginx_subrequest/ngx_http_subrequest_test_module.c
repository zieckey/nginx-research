...
static ngx_http_module_t  ngx_http_subrequest_test_module_ctx =
{
...
ngx_module_t  ngx_http_subrequest_test_module =
{
    NGX_MODULE_V1,
    &ngx_http_subrequest_test_module_ctx,           /* module context */
                                                         ...
                                                         void *data, ngx_int_t rc)
{
    ...
    myctx = ngx_http_get_module_ctx(pr, ngx_http_subrequest_test_module);
...
subrequest_test_post_handler(ngx_http_request_t * r)
{
    ...
    myctx = ngx_http_get_module_ctx(r, ngx_http_subrequest_test_module);
...
ngx_http_subrequest_test(ngx_conf_t * cf, ngx_command_t * cmd, void * conf)
{
    ...
    //首先找到subrequest_test配置项所属的配置块，clcf貌似是location块内的数据
    //结构，其实不然，它可以是main、srv或者loc级别配置项，也就是说在每个
    //http{}和server{}内也都有一个ngx_http_core_loc_conf_t结构体
    clcf = ngx_http_conf_get_module_loc_conf(cf, ngx_http_core_module);

    //http框架在处理用户请求进行到NGX_HTTP_CONTENT_PHASE阶段时，如果
    //请求的主机域名、URI与subrequest_test配置项所在的配置块相匹配，就将调用我们
    //实现的ngx_http_subrequest_test_handler方法处理这个请求
...
ngx_http_subrequest_test_handler(ngx_http_request_t * r)
{
    ...
    //创建http上下文
    ngx_http_subrequest_test_ctx_t* myctx = ngx_http_get_module_ctx(r, ngx_http_subrequest_test_module);
    if (myctx == NULL)
    {
        ...
        //将上下文设置到原始请求r中
        ngx_http_set_ctx(r, myctx, ngx_http_subrequest_test_module);
    }
    ...
    //设置子请求回调方法为subrequest_test_subrequest_post_handler
    psr->handler = subrequest_test_subrequest_post_handler;

    //data设为myctx上下文，这样回调subrequest_test_subrequest_post_handler
    //时传入的data参数就是myctx
    psr->data = myctx;
